...
export async function GET(_request, { params }) {
  const productName = (await params).productName; // This might be original name from URL
  ...
  try {
    ...
    const recentErrors = Status.getRecentErrors()
      .filter(
        (err) =>
          err.message.includes(productName) ||
          err.message.includes(sanitizedName)
      )
    ...
    console.error(
      `Error checking processing status for ${productName}:`,
      error
    );
    return NextResponse.json(
