...
// Improved Mutex implementation that properly supports async/await
class Mutex {
  constructor() {
    this.locked = false;
    this.waitQueue = [];
...
async function processQueueItem() {
  ...
  // Dequeue safely
  await queueLock.lock(); // Use await with our async mutex lock
  try {
  ...
  if (productOriginalName) {
    ...
    try {
      ...
      Status.addErrorLog(
        `Background processing error for ${productOriginalName}: ${
...
async function processingLoop() {
  ...
  while (!stopProcessingLoop && !Status.shutdown_event.isSet()) {
    ...
    if (queueNotEmpty) {
      ...
      await new Promise((resolve) => setTimeout(resolve, 200)); // Short delay between items to prevent tight loops
    } else {
      // Wait longer if the queue is empty
      await new Promise((resolve) => setTimeout(resolve, 2000)); // Check queue every 2 seconds
    }
  }
  isProcessorLoopRunning = false;
  log(
    `Processing loop stopped (Stop flag: ${stopProcessingLoop}, Shutdown: ${Status.shutdown_event.isSet()}).`
  );
}
...
export function enqueueProductProcessing(productOriginalName) {
  ...
  try {
    ...
    if (!processingQueue.includes(productOriginalName)) {
      ...
      log(
        `Enqueued: ${productOriginalName}. Queue size: ${processingQueue.length}`
      );
      wasEnqueued = true;
    } else {
  ...
  if (wasEnqueued && !isProcessorLoopRunning && !stopProcessingLoop) {
    ...
    processingLoop().catch((err) => {
      ...
      Status.addErrorLog(
        `Fatal background processing loop error: ${
