...
import { addErrorLog } from "./status";

// Import server initialization
if (typeof window === "undefined") {
  import("./server-init").catch(console.error);
}
...
export async function listProductFolders() {
  ...
  try {
    ...
    try {
      ...
      console.error(
        `Failed to create input directory: ${Config.INPUT_DIR}`,
        err
      );
    }
    ...
    if (error instanceof Error && error.code === "ENOENT") {
      ...
      // Try to create it again just in case
      try {
        await fs.mkdir(Config.INPUT_DIR, { recursive: true });
        console.log(`Created input directory: ${Config.INPUT_DIR}`);
        // Return empty array as the directory was just created
        return [];
      } catch (mkdirErr) {
        console.error(
          `Failed to create input directory after read attempt: ${Config.INPUT_DIR}`,
          mkdirErr
        );
      }
    } else {
...
export async function listVariantFolders(productOriginalName) {
  ...
  try {
    ...
    try {
      ...
      console.error(`Failed to create product directory: ${productPath}`, err);
    }
    ...
    if (error instanceof Error && error.code === "ENOENT") {
      ...
      console.warn(
        `Product input directory not found for listing variants: ${productPath}`
      );
      // Try to create it
      try {
        await fs.mkdir(productPath, { recursive: true });
        ...
        console.error(
          `Failed to create product directory: ${productPath}`,
          mkdirErr
        );
      }
    } else {
 ...
 * @property {Record<string, Array<{width: number, height: number, s3Key: string}>>} dimensionsPerSize - e.g., { lg: [{width: 1600, height: 1200, s3Key: "..."}], md: [...] }
 */

/**
 /**
 * Generates the final JSON object structured for Sanity using local file paths.
 ...
 * @param {ProcessedVariantInfo[]} processedVariantsInfo - Array of results containing aggregated local paths per size.
 * @returns {Promise<object>} The final JSON object.
 */
export async function generateSanityJsonLocal(
  ...
  for (const variantInfo of processedVariantsInfo) {
    ...
    // Map processed local paths to JSON breakpoints
    for (const bpKey of Config.SORTED_BREAKPOINT_KEYS) {
      const sizeKey = Config.JSON_BREAKPOINTS[bpKey];
      const localPaths =
        variantInfo.processedLocalPathsPerSize?.[sizeKey] ?? [];

      // Store path array in the imageSets
      imageSets[bpKey] = localPaths;

      // Get dimensions for this breakpoint
      const dimensions = variantInfo.dimensionsPerSize?.[sizeKey] ?? [];
      dimensionsPerSize[bpKey] = dimensions.map((d) => ({
        width: d.width || 0,
        height: d.height || 0,
        path: d.outputPath || "",
        s3Key: d.s3Key || "",
    ...
    if (variantHasImages) {
      finalVariants.push({
        ...
        imageSets: imageSets,
        dimensions: dimensionsPerSize, // Include dimensions in the output JSON
      });
    } else {
      console.warn(
  ...
  const finalJson = {
    ...
    localFilesOnly: true, // Flag to indicate this JSON has local paths, not S3 URLs
  };
