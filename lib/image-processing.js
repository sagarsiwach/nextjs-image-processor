...
async function processSingleImage(
  ...
  try {
    ...
    for (const sizeKey of Config.SIZE_KEYS) {
      ...
      // Create output directory path for this variant/size
      ...
      // Get S3 key (for reference, not used for upload now)
      ...
      // Log hotspot for debugging
      console.log(
        `[DEBUG] Processing ${sizeKey} with hotspot:`,
        JSON.stringify(currentHotspot)
      );
      ...
      try {
        ...
        console.log(`[DEBUG] Applying crop for ${sizeKey}:`, {
          left: cropLeft,
          top: cropTop,
          width: cropWidth,
          height: cropHeight,
          ...
          hotspotXPosition: Math.round(metadata.width * currentHotspot.x),
          hotspotYPosition: Math.round(metadata.height * currentHotspot.y),
        });

        // Apply crop based on hotspot, ensuring the parameters are valid
        processor = processor.extract({
          left: Math.floor(cropLeft),
          top: Math.floor(cropTop),
          width: Math.min(
            Math.floor(cropWidth),
            metadata.width - Math.floor(cropLeft)
          ),
          height: Math.min(
            Math.floor(cropHeight),
            metadata.height - Math.floor(cropTop)
          ),
        });
        ...
        // Create output directory and save the file locally
        await fs.mkdir(outputDir, { recursive: true });
        await fs.writeFile(outputPath, outputBuffer);

        console.log(`   ✅ Saved to local: ${outputPath}`);

        resultsForThisImage[sizeKey] = {
          success: true,
          outputPath: outputPath,
          s3Key: s3Key, // Store S3 key for reference
