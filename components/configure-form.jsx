...
import { HotspotSelector } from './hotspot-selector';
...
export function ConfigureForm({
    ...
    // Form State
    const [title, setTitle] = useState(initialConfig?.title || Config.DEFAULT_TITLE);
    const [subtitle, setSubtitle] = useState(initialConfig?.subtitle || Config.DEFAULT_SUBTITLE);
    const [variants, setVariants] = useState(initialVariants || []);
    const [hotspots, setHotspots] = useState(initialConfig?.hotspots || deepClone(Config.DEFAULT_HOTSPOTS_CONFIG)); // Use deepClone

    // Helper for deep cloning defaults safely in JS
    function deepClone(obj) {
        try { return JSON.parse(JSON.stringify(obj)); }
        catch (e) { console.error("Deep clone failed:", e); return {}; }
    }


    // Callback for HotspotSelector component
    ...
    const handleSubmit = async (event) => {
        ...
        startTransition(async () => {
             ...
             const payload = {
                 ...
                 subtitle,
                 hotspots,
                 variants: variants.map(v => ({ // Send necessary variant data for saving/reference if needed by API
            ...
            try {
                const response = await fetch(`/api/configure/${encodeURIComponent(productOriginalName)}`, { // Use ORIGINAL name in URL to save config correctly
                    method: 'POST',
                ...
                toast.success(`Configuration saved for ${productOriginalName}. Processing started...`);
     ...
     useEffect(() => {
         ...
         if (isProcessing) {
             ...
             intervalId = setInterval(async () => {
                 try {
                     const res = await fetch(`/api/product/${encodeURIComponent(productSanitizedName)}/status`); // Poll using SANITIZED name
                     if (res.ok) {
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-white p-4 md:p-6 rounded shadow-md space-y-6">
            {/* General Config */}
            <fieldset className="border p-4 rounded border-border"> {/* Use border-border for consistency */}
                <legend className="text-lg font-semibold px-2 mb-2 text-foreground">General</legend>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <Label htmlFor="subtitle" className="text-sm font-medium mb-1 block">Subtitle</Label>
                        <Input
                            id="subtitle" name="subtitle" value={subtitle}
            ...
            <fieldset className="border p-4 rounded border-border">
                <legend className="text-lg font-semibold px-2 mb-2 text-foreground">Hotspot Selection Per Size</legend>
                <p className="text-sm text-muted-foreground mb-4">Click on the image for each size group to set its focal point.</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                 ...
                 {!sampleImageUrl && <p className="text-destructive text-sm mt-4">Cannot select hotspots: No sample image found.</p>}
            </fieldset>

            {/* Variants Config */}
            <fieldset className="border p-4 rounded border-border">
                <legend className="text-lg font-semibold px-2 mb-2 text-foreground">Variants Configuration</legend>
                <p className="text-sm text-muted-foreground mb-4">Configure display names and color swatches for detected variants.</p>
                <div className="space-y-3">
                    {variants.length > 0 ? variants.map((variant, index) => (
                        <div key={variant.original_name} className="grid grid-cols-[1fr,auto,auto] sm:grid-cols-[1fr,2fr,auto] items-center gap-3 border-b pb-2 last:border-b-0 border-border">
                            <Label htmlFor={`variant-name-${index}`} className="text-sm font-medium text-foreground truncate pr-2" title={variant.original_name}>
                                {variant.original_name}
                            </Label>
                            {/* Hidden input to track original name during POST */}
                            <input type="hidden" name={`variants[${index}][original_name]`} value={variant.original_name} />
                            <Input
                                id={`variant-name-${index}`}
                                name={`variants[${index}][display_name]`}
                                value={variant.display_name}
                                onChange={(e) => handleVariantChange(index, 'display_name', e.target.value)}
                            ...
                            <Input
                                ...
                                onChange={(e) => handleVariantChange(index, 'colorValue', e.target.value)}
                                title="Select color"
                       ...
                       <p className="text-destructive text-sm">No variant subdirectories found in the input folder.</p>
                    )}
                </div>
            </fieldset>
            ...
            <div className="mt-6 pt-4 border-t border-border">
                ...
                >
                    {isProcessing ? 'Processing...' : (isPending ? 'Saving...' : 'Save Configuration & Start Processing')}
                </Button>
                {isProcessing && <p className="text-xs text-orange-600 mt-2 text-center">Processing running... You can leave this page. Check console or status API for updates.</p>}
            </div>
        </form>
    );
}